...
def train(params_file="./keras_to_loihi_params", epochs=1, **kwargs):
    ...
    with nengo_dl.Simulator(converter.net, seed=0, minibatch_size=100) as sim:
        sim.compile(
            ...
            loss={
                converter.outputs[output]: tf.losses.SparseCategoricalCrossentropy(
                    from_logits=True
                )
            },
            metrics={converter.outputs[output]: tf.metrics.sparse_categorical_accuracy},
        )
        sim.fit(
            {converter.inputs[inputs]: X_train},
            {converter.outputs[output]: X_train},
            epochs=epochs,
        )
