...
netParams = snn.params('network_anomaly_supervised.yaml')
...
def genLoihiParams(net):
    ...
    np.save('Outputs/Fc1.npy', fc1Weights)
    np.save('Outputs/Fc2.npy', fc2Weights)
...
if __name__ == '__main__':
    ...
    for epoch in range(100):
        ...
        stats.update()
        stats.plot(saveFig=True, path='Outputs/')
        if stats.training.bestLoss is True:
            torch.save(net.state_dict(), 'Outputs/nmnistNet.pt')

    # Save training data
    stats.save('Outputs/')
    net.load_state_dict(torch.load('Outputs/nmnistNet.pt'))
    genLoihiParams(net)
