...
def TPR_FPR_arrays_doubledetector(noise_array_L1, injection_array_L1, noise_array_H1, injection_array_H1, model_outdir, steps, num_entries=100): 
    ...
    if badremoved:     
        model_L1_signal = load_model('%s/best_badremoved_standard_l1_injection.hdf5'%(model_outdir))
        model_H1_signal = load_model('%s/best_badremoved_standard_h1_injection.hdf5'%(model_outdir))
        model_H1_noise = load_model('%s/best_badremoved_standard_h1_noise.hdf5'%(model_outdir))
        model_L1_noise = load_model('%s/best_badremoved_standard_l1_noise.hdf5'%(model_outdir))
    
    n_noise_events = np.shape(noise_array_L1)[0]
    n_signal_events = np.shape(injection_array_L1)[0]
    ...
    X_pred_noise_AEnoise_L1 = model_L1_noise.predict(noise_array_L1)
    X_pred_noise_AEnoise_H1 = model_H1_noise.predict(noise_array_H1)
    X_pred_noise_AEsignal_L1 = model_L1_signal.predict(noise_array_L1)
    X_pred_noise_AEsignal_H1 = model_H1_signal.predict(noise_array_H1)
    ...
    # Determine thresholds for FPR quantiles
    loss_fn = MeanSquaredError(reduction='none')
    losses_L1_AEnoise = loss_fn(noise_array_L1, X_pred_noise_AEnoise_L1).numpy()
    losses_H1_AEnoise = loss_fn(noise_array_H1, X_pred_noise_AEnoise_H1).numpy()
    losses_L1_AEsignal = loss_fn(noise_array_L1, X_pred_noise_AEsignal_L1).numpy()
    losses_H1_AEsignal = loss_fn(noise_array_H1, X_pred_noise_AEsignal_H1).numpy()
    
    del X_pred_noise_AEnoise_L1, X_pred_noise_AEnoise_H1, X_pred_noise_AEsignal_L1, X_pred_noise_AEsignal_H1, noise_array_L1, noise_array_H1
        
    averaged_losses_L1_AEnoise = np.mean(losses_L1_AEnoise, axis=1).reshape(n_noise_events, -1)
    averaged_losses_H1_AEnoise = np.mean(losses_H1_AEnoise, axis=1).reshape(n_noise_events, -1)
    averaged_losses_L1_AEsignal = np.mean(losses_L1_AEsignal, axis=1).reshape(n_noise_events, -1)
    averaged_losses_H1_AEsignal = np.mean(losses_H1_AEsignal, axis=1).reshape(n_noise_events, -1)
    
    fig = plt.figure()
    ax = Axes3D(fig)
    
    max_loss_L1_AEnoise = [np.max(event_L1) for event_L1 in averaged_losses_L1_AEnoise]
    max_loss_H1_AEnoise = [np.max(event_H1) for event_H1 in averaged_losses_H1_AEnoise]
    max_loss_L1_AEsignal = [np.max(event_L1) for event_L1 in averaged_losses_L1_AEsignal]
    max_loss_H1_AEsignal = [np.max(event_H1) for event_H1 in averaged_losses_H1_AEsignal]
    
    ax.scatter(max_loss_H1_AEnoise, max_loss_L1_AEsignal, max_loss_H1_AEsignal, alpha = 0.5, color = 'blue', label = 'noise')
    '''
    ...
    print('Evaluating Model on test data. This make take a while...')
    X_pred_signal_AEnoise_L1 = model_L1_noise.predict(injection_array_L1)
    X_pred_signal_AEnoise_H1 = model_H1_noise.predict(injection_array_H1)
    X_pred_signal_AEsignal_L1 = model_L1_signal.predict(injection_array_L1)
    X_pred_signal_AEsignal_H1 = model_H1_signal.predict(injection_array_H1)
    ...
    losses_L1_AEnoise = loss_fn(injection_array_L1, X_pred_signal_AEnoise_L1).numpy()
    losses_H1_AEnoise = loss_fn(injection_array_H1, X_pred_signal_AEnoise_H1).numpy()
    losses_L1_AEsignal = loss_fn(injection_array_L1, X_pred_signal_AEsignal_L1).numpy()
    losses_H1_AEsignal = loss_fn(injection_array_H1, X_pred_signal_AEsignal_H1).numpy()
    
    del X_pred_signal_AEnoise_L1, X_pred_signal_AEnoise_H1, X_pred_signal_AEsignal_L1, X_pred_signal_AEsignal_H1, injection_array_L1, injection_array_H1
    
    averaged_losses_L1_AEnoise = np.mean(losses_L1_AEnoise, axis=1).reshape(n_signal_events, -1)
    averaged_losses_H1_AEnoise = np.mean(losses_H1_AEnoise, axis=1).reshape(n_signal_events, -1)
    averaged_losses_L1_AEsignal = np.mean(losses_L1_AEsignal, axis=1).reshape(n_signal_events, -1)
    averaged_losses_H1_AEsignal = np.mean(losses_H1_AEsignal, axis=1).reshape(n_signal_events, -1)
    ...
    ax.scatter(max_loss_H1_AEnoise, max_loss_L1_AEsignal, max_loss_H1_AEsignal, alpha = 0.5, color = 'red', label = 'injection')
    
    ax.set_xlabel('H1 Noise Loss')
    ax.set_ylabel('L1 Signal Loss')
    ax.set_zlabel('H1 Signal Loss')
    plt.legend()
    plt.savefig(outdir+'/3dspace_other.jpg')
...
def main(args):
    ...
    if clip: 
        ...
        X_test_L1, X_test_H1 = clean(load_signal['injection_samples']['l1_strain'][:datapoints, :int(2048*5.55)],
                                     load_signal['injection_samples']['h1_strain'][:datapoints, :int(2048*5.55)],  200)
        X_train_L1, X_train_H1 = clean(load_noise['noise_samples']['l1_strain'][:datapoints, :int(2048*5.55)], 
                                       load_noise['noise_samples']['h1_strain'][:datapoints, :int(2048*5.55)], 200)
    ...
    X_test_L1 = scaler_l1.transform(X_test_L1.reshape((-1, 1))).reshape((-1, int(2048*5.55)))
    X_train_L1 = scaler_l1.transform(X_train_L1.reshape((-1, 1))).reshape((-1,int(2048*5.55)))
    X_test_H1 = scaler_h1.transform(X_test_H1.reshape((-1, 1))).reshape((-1, int(2048*5.55)))
    X_train_H1 = scaler_h1.transform(X_train_H1.reshape((-1, 1))).reshape((-1, int(2048*5.55)))
